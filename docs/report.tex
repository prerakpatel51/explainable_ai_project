...
A common problem in deploying deep learning models is \textbf{domain shift}:
performance drops when a model trained on one data distribution is applied to a
different but related one.  A medical imaging model trained at one hospital may
fail on images from another; an autonomous driving system trained in sunny
conditions may struggle in rain.  Understanding \emph{how} a model's reasoning
changes under domain shift matters as much as measuring the accuracy drop,
because it tells us whether the model has learned transferable concepts or just
...
overlay.  This section presents qualitative visualisations followed by
quantitative evaluation along four axes: stability, faithfulness,
cross-domain consistency, and representation analysis.

% ═══════════════════════════════════════════════════════════════════════════
\subsection{In-Domain Attribution Maps}
\label{sec:in_domain_attr}

We first examine how the four XAI methods explain model predictions on
\emph{in-domain} data, real images explained by the real model, and
sketch images explained by the sketch model.
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{LIME}
\end{subfigure}
\caption{\textbf{Real tiger, real model} (98.36\% accuracy).
Grad-CAM and Grad-CAM++ focus on the face and striped body, and the model
...
important pixels.  LIME identifies superpixel regions covering the body and
background boundary.  This confirms the real model leverages \emph{texture
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{LIME}
\end{subfigure}
\caption{\textbf{Real ladder, real model.}
Grad-CAM highlights the central rung and rail region.  Grad-CAM++ provides
broader coverage of the A-frame structure.  Integrated Gradients picks up fine
edge details along individual rungs.  LIME identifies the ladder outline as
the key superpixel region.  All methods agree on semantically meaningful
features, an example of high-quality, consistent explanations.}
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{LIME}
\end{subfigure}
\caption{\textbf{Real scissors, real model} (98.25\% accuracy).
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{LIME}
\end{subfigure}
\caption{\textbf{Real lighthouse, real model} (95.83\% accuracy).
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Integrated Gradients}
\end{subfigure}
\caption{\textbf{Real lion, real model} (98.08\% accuracy).
Grad-CAM highlights the face and mane region as a broad hot zone.  Integrated
Gradients provides finer detail, picking up specific facial features like the
eyes, nose, and mane edges.}
\label{fig:attr_lion}
\end{figure}

% ── Lighthouse: sketch model, sketch domain ─────────────────────────────
\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{LIME}
\end{subfigure}
\caption{\textbf{Sketch lighthouse, sketch model.}
On sketches, Grad-CAM focuses on the tower silhouette rather than texture
details, consistent with the sketch model learning structural features.
Integrated Gradients traces specific line strokes of the drawing.  LIME
highlights the superpixel regions covering the tower and light structure.}
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{LIME}
\end{subfigure}
\caption{\textbf{Sketch saxophone, sketch model.}
...
% ═══════════════════════════════════════════════════════════════════════════
\subsection{Cross-Model Comparison on Sketch Images}
\label{sec:cross_model_sketch}

A central question is how domain shift affects explanation quality.  Here we
compare the real model and sketch model on the \emph{same sketch images}.
The sketch model was trained on sketches and should produce focused, confident
attributions; the real model has never seen sketches and may struggle.
...
\end{subfigure}
\caption{\textbf{Sketch bicycle: real model vs.\ sketch model.}
...
The real model struggles with the sketch, and its heatmap is diffuse and may
...
% ═══════════════════════════════════════════════════════════════════════════
\subsection{Cross-Model Comparison on Real Images}
\label{sec:cross_model_real}

While the previous subsection showed how models differ on sketch images,
it is equally important to compare explanations on \emph{real} photographs.
Even when both models achieve high accuracy on real images, the underlying
reasoning, as revealed by attribution maps, can differ substantially.

% ── Real tiger: real model vs. sketch model ──────────────────────────────
\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/real_model/attribution_maps/gradcam/real/tiger_1405.png}
...
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/sketch_model/attribution_maps/gradcam/real/tiger_1405.png}
    \caption{Sketch model, Grad-CAM}
\end{subfigure}
...
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/sketch_model/attribution_maps/lime/real/tiger_1405.png}
    \caption{Sketch model, LIME}
\end{subfigure}
\caption{\textbf{Real tiger: real model vs.\ sketch model.}
On a real photograph, the real model (left) produces texture-focused
attributions, attending to the tiger's stripes, fur patterns, and colour
gradients.  The sketch model (right) focuses more on the overall \emph{shape}, specifically the body outline and silhouette, because it learned shape-based features
from line drawings.  Both models correctly classify the image, but their
reasoning strategies are fundamentally different.  This demonstrates that
high accuracy alone does not guarantee consistent explanations.}
\label{fig:attr_tiger_real_cross}
\end{figure}

% ── Real lion: real model vs. sketch model ───────────────────────────────
\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/real_model/attribution_maps/gradcam/real/lion_0658.png}
...
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/sketch_model/attribution_maps/gradcam/real/lion_0658.png}
    \caption{Sketch model, Grad-CAM}
\end{subfigure}
...
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/real_model/attribution_maps/lime/real/lion_0658.png}
    \caption{Real model, LIME}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/sketch_model/attribution_maps/lime/real/lion_0658.png}
    \caption{Sketch model, LIME}
\end{subfigure}
\caption{\textbf{Real lion: real model vs.\ sketch model.}
The real model highlights the mane's texture and facial detail, features
rich in colour and texture gradients.  The sketch model focuses on the mane's
\emph{silhouette} and the face outline.  Despite both models achieving
similar accuracy on this class, the sketch model's shape-focused attributions
are more spatially compact and consistent, confirming that shape-based
representations transfer more cleanly across domains.}
\label{fig:attr_lion_real_cross}
\end{figure}

% ── Real bicycle: all 4 methods, real model ──────────────────────────────
\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/real_model/attribution_maps/gradcam/real/bicycle_0201.png}
    \caption{Grad-CAM}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/real_model/attribution_maps/gradcam_pp/real/bicycle_0201.png}
    \caption{Grad-CAM++}
\end{subfigure}
...
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/real_model/attribution_maps/integrated_gradients/real/bicycle_0201.png}
    \caption{Integrated Gradients}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/real_model/attribution_maps/lime/real/bicycle_0201.png}
    \caption{LIME}
\end{subfigure}
\caption{\textbf{All four methods on a real bicycle} (real model).
Grad-CAM and Grad-CAM++ highlight the wheels and frame as broad spatial
regions.  Grad-CAM++ provides fuller coverage of both wheels compared to
Grad-CAM's tighter focus.  Integrated Gradients picks up fine structural
edges including spokes, pedals, and handlebar outlines.  LIME identifies the
wheels and frame as the most important superpixel regions.  All methods
converge on the same key features, demonstrating strong inter-method
agreement on geometrically distinctive classes.}
\label{fig:attr_bicycle_real}
\end{figure}

% ── Real banana: all 4 methods, sketch model ─────────────────────────────
\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/sketch_model/attribution_maps/gradcam/real/banana_0100.png}
    \caption{Grad-CAM}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/sketch_model/attribution_maps/gradcam_pp/real/banana_0100.png}
    \caption{Grad-CAM++}
\end{subfigure}
...
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/sketch_model/attribution_maps/integrated_gradients/real/banana_0100.png}
    \caption{Integrated Gradients}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/sketch_model/attribution_maps/lime/real/banana_0100.png}
    \caption{LIME}
\end{subfigure}
\caption{\textbf{All four methods on a real banana} (sketch model).
Despite being trained exclusively on sketches, the sketch model produces
clean, focused attributions on the real banana photograph.  The banana's
simple curved shape is a feature the sketch model learned from line drawings,
and it transfers perfectly to the real domain.  This is a compelling
example of shape-based transfer: the model has never seen a real banana
during training, yet its explanations are as coherent and focused as those
of the real model on in-domain images.}
\label{fig:attr_banana_real_sketchmodel}
\end{figure}

% ── Real bicycle: cross-model comparison ─────────────────────────────────
\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/real_model/attribution_maps/gradcam/real/bicycle_0201.png}
...
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/sketch_model/attribution_maps/gradcam/real/bicycle_0201.png}
    \caption{Sketch model, Grad-CAM}
\end{subfigure}
...
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{output/xai/sketch_model/attribution_maps/lime/real/bicycle_0201.png}
    \caption{Sketch model, LIME}
\end{subfigure}
\caption{\textbf{Real bicycle: real model vs.\ sketch model.}
The bicycle is a geometrically distinctive class with circular wheels and
a triangular frame.  Both models focus on the same structural features,
but the sketch model's attributions are tighter and more shape-focused, as it highlights the wheel outlines and frame geometry specifically.  The real
model's attributions are broader, incorporating texture and colour information
from the background and surface details.  This confirms that shape-based
features (learned from sketches) produce more precise, interpretable
explanations even on photographs.}
\label{fig:attr_bicycle_real_cross}
\end{figure}
...
confirms that the sketch model learns \emph{domain-invariant} representations where real and sketch images of the same class are mapped to similar locations
...
The clearest result is the \textbf{asymmetric transfer} between domains.
Sketch$\to$real transfer (83.02\%) far outperforms real$\to$sketch (56.17\%).
This fits the \emph{texture bias} hypothesis: CNNs trained on photographs rely
on texture cues (colour gradients, surface patterns) that are absent in line
drawings.  Sketch training, on the other hand, forces the model to use
\emph{shape-based} features (edges, contours, spatial relationships) that exist
in both domains.
...
The cross-domain attribution comparisons (Figures~\ref{fig:attr_cross_bicycle},
\ref{fig:attr_banana_cross}, \ref{fig:attr_tiger_cross}) and the faithfulness
results (Section~\ref{sec:faithfulness}) show that \textbf{domain shift
degrades explanation quality}, not just classification accuracy.  When the real
model is applied to sketches:
...
The sketch model's attributions on real images, by contrast, stay relatively
...
(Section~\ref{sec:representations}) offers a simple explanation: the sketch
model's feature space mixes domains, while the real model separates them.
...
All source code for this project (model training, XAI attribution generation,
evaluation pipelines, and visualisation scripts) is publicly available:

\begin{itemize}[leftmargin=*]
    \item \textbf{Project Repository:} \url{https://github.com/prerakpatel51/explainable_ai_project}
    \item \textbf{Dataset (DomainNet):} \url{https://ai.bu.edu/DomainNet/}
\end{itemize}
...
We evaluated four XAI attribution methods on ResNet-152 classifiers trained on
two visual domains from DomainNet.  Our main findings:
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Sketch-trained models generalise better:}  The sketch model
          ...
          model transfers to sketches (56.17\%).  Shape-based representations
          are more domain-invariant than texture-based ones.

    \item \textbf{Domain shift degrades explanations:}  Classification accuracy
          drops under domain shift, and so does explanation quality, and attributions become diffuse, unfocused, and less semantically
          meaningful.
    ...
    \item \textbf{LIME is the most faithful:}  Despite low stability, LIME
          achieves the highest insertion AUC ($\approx 0.57$--$0.61$).  Its
          superpixel-based explanations capture the features the model actually
          relies on better than the other methods.

    \item \textbf{Granularity-stability trade-off:}  Pixel-level methods
          (Integrated Gradients, LIME) give finer spatial detail but are less
          stable under input perturbation (SSIM $\approx 0.47$--$0.56$).
    ...
    \item \textbf{Domain-invariant representations explain transfer:}
          ...
          This directly accounts for the asymmetric transfer performance.

    \item \textbf{No single XAI method wins everywhere:}  Each method has
          strengths in different axes.  Evaluating explanations requires
          measuring stability, faithfulness, cross-domain consistency, and
          representation structure together.
\end{enumerate}
